...
const ExpectedErrors = error{TooShortDictionarySize};

/// `dictionary` の長さをもとに、ビットマスクを計算します
/// > 辞書文字列の長さをそのままモジュロ計算すると、公平性(統計的バイアス)の問題が発生するため、長さをそれに近い2進数に切り上げる
pub fn calculateBitsMask(dictionary_len: usize) ExpectedErrors!usize {
    if (dictionary_len <= 1) return error.TooShortDictionarySize;
...
pub fn calculateBatchSize(dictionary_len: usize, required_len: usize, mask: usize) ExpectedErrors!usize {
    if (dictionary_len <= 1) return error.TooShortDictionarySize;
